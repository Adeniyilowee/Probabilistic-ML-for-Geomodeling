...
import calculation
import visualize, visualize2, visualize3, visualize4, visualize_all
from simulation import probability, information_entropy, fuzziness
from geophysics_data import geophysics_data, geophysics_data_, geophysics_data_final
#from simulation_MCMC import gaussian_pymc
from simulation_MCMC2 import Bayesian_Inference_Analysis as bay_inf
import numpy as np
...
class Fit(object):
    ...
    def subsurfmodel(self, a, b, ctr_points_plot, dimension, knots_list_u, knots_list_v, degree=3): # ctr_points not needed, same with the reshaped ctr points from flattened
    ...
    def volumetric_mesh(self, density, layer=1):
        ...
        if layer == 1:
            ...
            #print(np.array(self.data['surf_2']).shape)
        ...
        plotter.show()
        '''
    def trimesh_it(self, mesh):
        mesho = self.surf_recon(mesh)
        mesh = mesho.delaunay_3d(alpha=5)

        #mesh.triangulate(inplace=True)
        mesh.point_data['point_inds'] = range(mesh.n_points)
        mesh.cell_data['cell_inds'] = range(mesh.n_cells)
        mesh_edges = mesh.extract_feature_edges()
        mesh_edges = mesh_edges.clip('z', origin=mesh.center).clip('-z', origin=mesh.center)
        mesh_edge_cells = mesh.extract_points(mesh_edges['point_inds'])
        return mesh_edge_cells, mesho

    def combine_mesh(self):
        theme = pv.themes.DocumentTheme()
        theme.show_edges = True

        mesh1_edge_cells, mesho_1 = self.trimesh_it(self.data['surf_1_'])
        mesh2_edge_cells, mesho_2 = self.trimesh_it(self.data['surf_2_'])
        ...
        # merge meshes
        merged = mesho_1 + edge_strip + mesho_2
    ...
    def surf_voxelization(self):
        ...
        layer_of_interest = self.data['points_1'] # this is for surface voxelization, also can be used in ML
        df = pd.DataFrame(data=layer_of_interest, columns=['x','y','z'])
        cloud = PyntCloud(df)
        voxelgrid_id = cloud.add_structure('voxelgrid', n_x =500, n_y = 500, n_z = 500) # equivalant to dx, dy, dz in gempy
        voxelgrid = cloud.structures[voxelgrid_id]
        Binary_voxel_array = voxelgrid.get_feature_vector(mode='binary')
        plt.imshow(Binary_voxel_array[:, 150, :], cmap='gray')
        plt.show()

    ################################################ finite volume 1 ######################################################    
    def vol_voxelization(self):
        # This is create if you want to see individual grids the target lithology and the others
        density = self.data['mesh'].length / 200
        if not pv.is_pyvista_dataset(self.data['mesh']):
            self.data['mesh'] = pv.wrap(self.data['mesh'])

        x_min, x_max, y_min, y_max, z_min, z_max = self.data['mesh'].bounds
        ...
        # Also extracted surface points from hexahedral cell was used see link below, hence the extracted surface does not form a mesh
        # https://docs.pyvista.org/examples/01-filter/extract-surface.html
        selection = ugrid.select_enclosed_points(self.data['mesh'].extract_surface(), tolerance=0.0, check_surface=False)
        selection_ = ugrid.select_enclosed_points(self.data['mesh'].extract_surface(), tolerance=0.0, check_surface=False, inside_out=True)
        mask = selection.point_data['SelectedPoints'].view(np.bool_)
        mask_ = selection_.point_data['SelectedPoints'].view(np.bool_)
        ugrid['density'] = np.arange(ugrid.n_cells) # a
        #ugrid['density'] = np.full(ugrid.n_cells, 3.30) # or b
        vox_in = ugrid.extract_points(mask)
        vox_out = ugrid.extract_points(mask_)

        # plotting part
        p = pv.Plotter(notebook=False)
        p.add_mesh(ugrid, color=True, show_edges=True, scalars='density') # 1
        #p.add_mesh(vox_in, color=True, show_edges=True, scalars='density') # 2
        #p.add_mesh(vox_out, color=True, show_edges=True, scalars='density')  # 3 # with or without scalars
        p.show()
    ...
    def vol_viz(self):
        # This is for the visualization of the data points / sand body lol
        df = pd.DataFrame(data=self.data['mesh'].points, columns=['x', 'y', 'z'])
        points = pvg.points_to_poly_data(df)
        points.plot(clim=[0, 1], point_size=1, color='magenta')
        #print(self.data['mesh'].points)

    

    def surf_recon(self, mesh):
        # https://github.com/pyvista/pyvista/issues/1614
        # https://dev.pyvista.org/examples/01-filter/surface_reconstruction.html?highlight=surface%20reconstruction
        # edges = self.data['mesh'].extract_surface()
        edges = mesh.extract_surface()
        points = edges.points
        points = pv.wrap(points)

        from pyvista import _vtk as vtk
        v_algorithm = vtk.vtkSurfaceReconstructionFilter()
        v_algorithm.SetInputDataObject(points)
        v_algorithm.Update()
        vtmc = vtk.vtkMarchingCubes()
        vtmc.SetInputDataObject(v_algorithm.GetOutput())
        vtmc.SetValue(0, 0.0)
        vtmc.Update()
        surface = pv.wrap(vtmc.GetOutput())
        surface.clear_data()
        '''
        p = pv.Plotter()
        p.add_mesh(surface, color=True, show_edges=True)
        p.add_title('Reconstructed Surface')
        p.show()
        '''
        return surface
    ...
    def gaussian_MCMC(self, density):
        ...
        #initial_chain_state = [tf.constant(self.data['final_geo'], name='grav_initial', dtype=tf.float64)]
    ...
    def prior_top(self):
        ...
        fig = plt.gcf()
        fig.set_size_inches(14.0, 7.0)
        #fig.savefig('prior_top.png', dpi=300)
    ...
    def prior_posterior_top(self):
        ...
        #fig.savefig('prior_posterior.png', dpi=300)
        plt.show()
    ...
    def distribution_1(self, data, size):
        ...
        for i in range(len(data)):
            u = data[i]
            distribution = np.random.normal(0, 5, size=size)
            litr = []
            for j in range(len(distribution)):
                z = u.copy()  # wow, saved my life!
