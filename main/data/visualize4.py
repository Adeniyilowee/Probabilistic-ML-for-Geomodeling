...
def interactive_plotting(all_data, degree, a, b):
    ...
    model_subdivided_mesh_3 = all_data.subsurfmodel(a, b, all_data.data['ctr_points_plot'][3], all_data.data['dimension'][3], all_data.data['ctr_points'][3], all_data.data['knots_list_u'][3], all_data.data['knots_list_v'][3])
    ...
    all_data.data['surf_4_'] = model_subdivided_mesh_3
    all_data.data['surf_1'] = mesh_points(model_subdivided_mesh, dim)
    all_data.data['surf_2'] = mesh_points(model_subdivided_mesh_1, dim)
    all_data.data['surf_3'] = mesh_points(model_subdivided_mesh_2, dim)
    all_data.data['surf_4'] = mesh_points(model_subdivided_mesh_3, dim)
    end = time.time()
    print(end - start)
    point_ids = []
    selection = []
    ...
    # 2.3 Pyvista display setup 3
    def enable_sphere_widget2(check):
        def change_sphere_radius(value):
            pl.clear_sphere_widgets()
            radius_start[0] = value
            pl.add_sphere_widget(update_control_cage2, center=all_data.data['ctr_points_plot'][2], radius=value, color='yellow', test_callback=False)
    ...
    # 2.4 Pyvista display setup 4
    def enable_sphere_widget3(check):
        def change_sphere_radius(value):
            pl.clear_sphere_widgets()
            radius_start[0] = value
            pl.add_sphere_widget(update_control_cage3, center=all_data.data['ctr_points_plot'][3], radius=value, color='firebrick', test_callback=False)
    ...
    # 2.2.2. updating control cage when its points are moved/manipulated
    def update_control_cage3(point, idx):
        all_data.data['ctr_points_plot'][3][idx] = point
        dat = all_data.subsurfmodel(a, b, all_data.data['ctr_points_plot'][3], all_data.data['dimension'][3], all_data.data['ctr_points'][3], all_data.data['knots_list_u'][3], all_data.data['knots_list_v'][3])
        all_data.data['surf_4'] = mesh_points(dat, dim)
        all_data.data['surf_4_'] = dat
        pl.add_mesh(dat, color='darkslategray', use_transparency=False, show_edges=False, pickable=False, name='subdivided_surface4')
    ...
    pl.add_checkbox_button_widget(enable_sphere_widget2, size=30, color_on='yellow', position=((0.4 + 0) * pl.window_size[0], 10))
    pl.add_checkbox_button_widget(enable_sphere_widget3, size=30, color_on='firebrick', position=((0.45 + 0) * pl.window_size[0], 10))
    pl.add_mesh(data_points_mesh, color='aqua', point_size=4.0, render_points_as_spheres=True, name='data_points')
    pl.add_mesh(model_subdivided_mesh, color='sandybrown', use_transparency=False, show_edges=False, pickable=False, name='subdivided_surface1') # , scalars=model_subdivided_mesh[:].points[:, 2]
    pl.add_mesh(model_subdivided_mesh_1, color='darkgray', use_transparency=False, show_edges=False, pickable=False, name='subdivided_surface2')
    pl.add_mesh(model_subdivided_mesh_2, color='sienna', use_transparency=False, show_edges=False, pickable=False, name='subdivided_surface3')
    pl.add_mesh(model_subdivided_mesh_3, color='darkslategray', use_transparency=False, show_edges=False, pickable=False, name='subdivided_surface4')
